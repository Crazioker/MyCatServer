...
import com.mycat.util.StaticPeram;
...
public class GetCode extends HttpServlet {
	...
	public static int getPhonemsg(String phone) {
		...
		// 必填:短信签名-可在短信控制台中找到
		smsrequest.setSignName(StaticPeram.SignName);
		// 必填:短信模板-可在短信控制台中找到
		smsrequest.setTemplateCode(StaticPeram.TemplateCode);
		...
		try {
			...
			if (sendSmsResponse.getCode() != null && sendSmsResponse.getCode().equals("OK")) {
				...
				// 将获取的验证码和验证码有效时间存入数据库

				Date now = new Date();
				long time = 60 * 1000 * 5;
				SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");// 设置日期格式
				String eTime = df.format(new Date(now.getTime() + time));
				// String eTime=(String)(new Date(now.getTime()+time).getTime());
				...
				int res = userDao.insertCode(user);

				if (res == 1) {
					System.out.println("验证码更新到数据库成功！");
				} else {
					System.out.println("验证码更新到数据库失败！");
					return 2;
				}
